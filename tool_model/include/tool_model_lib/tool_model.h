...
std::string tool_model_pkg; //gives the path finding all the tool body parts inside the workspace
...
public:
    ...
    //cv::Mat homogeneous representation of face normals
     ...
     * Not currently used; replace by computeNoisedParticles
     * would be used  on initialization to generate random particles around the FK guess
    ...
    /**
       ...
       * Not currently used; PF motion model is now entirely in updateParticles
       * Would be used as particle filter's motion model (after initialization)
    ...
    /**
     * @brief Compute toolModel pose given a toolModel representation of joints 1-4 (cylinder pose) and a double representation of joints 5,6,7
     ...
     * Not currently used; would be called by gaussianSampling and used in particle filter's motion model
